...
mergeTCGAmaturemirna <- function(mirfileIDs, mirbasenames){
	
	testcounts<- lapply(mirfileIDs, function(x){
		# read datafile with isoforms
		tempfile <- read.table(file=paste("./", x ,"/isoforms.quantification.txt", sep=""), header=TRUE, stringsAsFactors=FALSE)
